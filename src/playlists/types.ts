...
/** QLC+ websocket message, e.g. "27|255". */
export type QlcMessage = string; // `${number}|${number}` | `FUNCTION|${number}|${number}`;

/** QLC+ messages to send at a single timestamp. */
export type QlcPlaylistEntry = QlcMessage[];

/** Playlist of QLC+ messages keyed by time strings. */
export type QlcPlaylist = Record<TimeString, QlcPlaylistEntry>;
...
export function assertTimeString(s: string): asserts s is TimeString {
  if (!TIME_STRING_REGEX.test(s)) {
    throw new Error(
      `Invalid time string: "${s}". Expected MM:SS, MM:SS.mmm, HH:MM:SS, or HH:MM:SS.mmm`,
    );
  }
}
